...
const BLANK: ProspectData = {
  firstName: '', businessName: '', cityArea: 'Newport Beach', industry: '',
  websiteUrl: '', referrerName: '', observation: '', templateId: 'cold', toEmail: '',
}

interface Props { onGenerate: (data: ProspectData) => void }
...
export function ProspectForm({ onGenerate }: Props) {
  ...
  return (
    <form onSubmit={e => { e.preventDefault(); onGenerate(data) }} className="flex flex-col gap-5 h-full">
      <div>
        <label className={labelClass}>Template</label>
        <select value={data.templateId} onChange={e => set('templateId', e.target.value as TemplateId)} className={inputClass}>
          {TEMPLATE_OPTIONS.map(t => <option key={t.id} value={t.id}>{t.label}</option>)}
        </select>
      </div>
      <div>
        <label className={labelClass}>Prospect Email</label>
        <input type="email" value={data.toEmail} onChange={e => set('toEmail', e.target.value)} placeholder="mike@acmeroofing.com" className={inputClass} />
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className={labelClass}>First Name</label>
          <input type="text" required value={data.firstName} onChange={e => set('firstName', e.target.value)} placeholder="Mike" className={inputClass} />
        </div>
        <div>
          <label className={labelClass}>Business Name</label>
          <input type="text" required value={data.businessName} onChange={e => set('businessName', e.target.value)} placeholder="Apex Roofing" className={inputClass} />
        </div>
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className={labelClass}>City / Area</label>
          <input type="text" value={data.cityArea} onChange={e => set('cityArea', e.target.value)} placeholder="Newport Beach" className={inputClass} />
        </div>
        <div>
          <label className={labelClass}>Industry</label>
          <select value={data.industry} onChange={e => set('industry', e.target.value as any)} className={inputClass}>
      ...
      <div>
        ...
        <input type="url" value={data.websiteUrl} onChange={e => set('websiteUrl', e.target.value)} placeholder="https://apexroofing.com" className={inputClass} />
      </div>
      {selectedTemplate?.requiresReferrer && (
        <div>
          <label className={labelClass}>Referrer Name</label>
          <input type="text" required value={data.referrerName} onChange={e => set('referrerName', e.target.value)} placeholder="Jane Smith" className={inputClass} />
        </div>
      )}
      <div className="flex-1">
        <div className="flex items-center justify-between mb-1.5">
          <label className={labelClass + ' mb-0'}>Personalized Observation</label>
          <button type="button" onClick={handleAiAssist} disabled={aiLoading || !data.firstName}
        ...
        <textarea value={data.observation} onChange={e => set('observation', e.target.value)}
      ...
      <div className="flex gap-3 pt-2">
        ...
        <button type="button" onClick={() => setData(BLANK)}
